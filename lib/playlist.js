    ...
    db.query(`SELECT * FROM playlist`, function(error,playlists){
        ...
        db.query(`SELECT * FROM playlist LEFT JOIN song ON playlist.id=song.playlist_id WHERE playlist.id=?`,[queryData.id], function(error2, playlist){
         ...
         while(i<playlist.length){      //song목록 불러오는과정
            if(!playlist[i].title)     //곡목록중에 title이 없으면 곡이 하나도 없다는 의미(title은 not null이기때문에)
            break;
...
exports.delete_process=function(request,response){
    var body = '';
      ...
      request.on('end', function(){
          var post = qs.parse(body);
          db.query(`DELETE FROM playlist WHERE id=?`, [post.id], function(error,result){
            if(error){
              throw error;
            }
            db.query(`DELETE FROM song WHERE playlist_id=?`,[post.id],function(error2, result2){
              if(error2){
                throw error2;
              }
            });
